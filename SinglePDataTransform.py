...
pathResultFile = "D:\\Daniel\\Documents\\magister\\eksperymenty\\data\\single_p_features.csv"
...
with open(pathResultFile, 'w', newline='') as resultfile:
    ...
    writer.writerow(["FE7XP", "FE7YP", "FE7ZP", "FE8XP", "FE8YP", "FE8ZP", "FE9XP", "FE9YP", "FE9ZP",
                     "FE10XP", "FE10YP", "FE10ZP", "FE11XP", "FE11YP", "FE11ZP", "F12XP", "F12YP", "F12ZP", "Y"])
    ...
    for filename in files:
        ...
        with open(pathDLA + "\\" + filename, 'r') as file:
            ...
            for row in rowp:
                ...
                y = row[1]
                z = row[2]
            writer.writerow([statistics.mean(xp), statistics.mean(yp), statistics.mean(zp),
                             ...
                             max(jerkxp), max(jerkyp), max(jerkzp), "0"])
    resultfile.close();
    ...
    for filename in files:
        ...
        with open(pathFalls + "\\" + filename, 'r') as file:
            ...
            writer.writerow([statistics.mean(xp), statistics.mean(yp), statistics.mean(zp),
                             ...
                             sum(np.absolute(xp)), sum(np.absolute(yp)), sum(np.absolute(zp)),
                             (max(xp) - min(xp)), (max(yp) - min(yp)), (max(zp) - min(zp)),
                             max(jerkxp), max(jerkyp), max(jerkzp), "1"])
    resultfile.close();